...
export const GravityBackground = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Mouse influence (now without)
      const targetX = cx + Math.sin(time * 0.5) * 20;
      const targetY = cy + Math.cos(time * 0.3) * 20;

      for (let r = 0; r < config.numRows; r++) {
        const rowProgress = r / config.numRows;
        const currentBaseRadius =
          config.innerRadius + rowProgress * config.thickness;

        ctx.fillStyle =
          r % 2 === 0 ? config.primaryColor : config.secondaryColor;
